...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        ...
        for iteration in range(20):
            ...
            # DONE: save additional metrics to be averaged
            metrics["train_acc"].append(dummy_train_accuracy)
            global_step += 1

        # DONE: log average train_accuracy
        logger.add_scalar("train_accuracy", torch.mean(torch.cat(metrics["train_acc"])), global_step)
        ...
        # DONE: log average val_accuracy
        logger.add_scalar("val_accuracy", torch.mean(torch.cat(metrics["val_acc"])), global_step)
