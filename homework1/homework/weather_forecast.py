...
class WeatherForecast:
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        # print(self.data)
        # print("mean diff")
        # print(self.data - self.data.mean(dim=1, keepdim=True))
        # print("absolute differences")
        # print((self.data - self.data.mean(dim=1, keepdim=True)).abs())
        # print("max of abs differences")
        # print((self.data - self.data.mean(dim=1, keepdim=True)).abs().max(dim=1))
        # print("values")
        # print((self.data - self.data.mean(dim=1, keepdim=True)).abs().max(dim=1).indices)
        return self.data[torch.arange(self.data.size(0)), (self.data - self.data.mean(dim=1, keepdim=True)).abs().max(dim=1).indices]
    ...
    def max_last_k_days(self, k: int) -> torch.Tensor:
        ...
        """
        #raise NotImplementedError
        return self.data.max(dim=1).values[-k:]
